...
fn main() {
    let displays = get_input();
    println!("easy outputs: {}", count_distinct_outputs(&displays));
    print!("Sum of values: {}", sum_values(&displays));
}

fn sum_values(v: &Vec<SegmentDisplay>) -> usize {
    v.iter().map(|dis| dis.value).sum()
}

fn count_distinct_outputs(v: &Vec<SegmentDisplay>) -> usize {
    let mut sum = 0;
    for dis in v {
        for val in dis.output {
...
impl SegmentDisplay {
    fn solve(&mut self) {
        self.decode_key[1] = self.inputs[0];
        self.decode_key[7] = self.inputs[1];
        self.decode_key[4] = self.inputs[2];
        self.decode_key[8] = self.inputs[9];

        self.inputs[0] = 0;
        self.inputs[1] = 0;
        self.inputs[2] = 0;
        self.inputs[9] = 0;
        //iterate over unsolved values with 5 segments active
        for i in 3..=5 {
            if self.inputs[i] & self.decode_key[7] == self.decode_key[7] {
                self.decode_key[3] = self.inputs[i];
                self.inputs[i] = 0;
                break;
            }
        }
        for i in 6..=8 {
            if self.inputs[i] & self.decode_key[3] == self.decode_key[3] {
                self.decode_key[9] = self.inputs[i];
                self.inputs[i] = 0;
                break;
            }
        }
        for i in 6..=8 {
            if (self.inputs[i] & self.decode_key[7]).count_ones() == 2 {
                self.decode_key[6] = self.inputs[i];
                self.inputs[i] = 0;
                break;
            }
        }
        for i in 3..=5 {
            if (self.inputs[i] & self.decode_key[4]).count_ones() == 2 {
                self.decode_key[2] = self.inputs[i];
                self.inputs[i] = 0;
                break;
            }
        }
        for i in 3..=5 {
            if self.inputs[i] != 0 {
                self.decode_key[5] = self.inputs[i];
                self.inputs[i] = 0;
                break;
            }
        }
        for (i, num) in self.inputs.iter_mut().enumerate() {
            if *num != 0 {
                self.decode_key[0] = *num;
                *num = 0;
            }
        }

        for (i, code) in self.output.iter().rev().enumerate() {
            for (j, decode) in self.decode_key.iter().enumerate() {
                if decode == code {
                    self.value += j * 10_usize.pow(i as u32);
                    break;
                }
            }
        }
...
impl std::fmt::Display for SegmentDisplay {
...
fn get_input() -> Vec<SegmentDisplay> {
    let mut v = vec![];
    let re = Regex::new(
        r"(?m)^(\w+).(\w+).(\w+).(\w+).(\w+).(\w+).(\w+).(\w+).(\w+).(\w+)...(\w+).(\w+).(\w+).(\w+)$",
    ).unwrap();

    let caps = re.captures_iter(INPUT);
    for cap in caps {
        let mut dis = SegmentDisplay {
            inputs: [
                to_u8(&cap[1]),
                to_u8(&cap[2]),
                to_u8(&cap[3]),
        ...
        dis.inputs.sort_by_key(|k| k.count_ones());
        dis.solve();
